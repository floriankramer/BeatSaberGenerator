...
_EASY_RAND_VALUES= {
  ...
  'tb_h_dir_left' : 0.5,
  'wall' : 0.005,
  'wall_length_sec' : [(0.3, 1), (0.6, 2), (1, 3)],
  'target_beat' : 70,
...
_NORMAL_RAND_VALUES = {
  ...
  'tb_h_dir_left' : 0.5,
  'wall' : 0.015,
  'wall_length_sec' : [(0.1, 1), (0.3, 2), (0.5, 2.5), (0.7, 3.5), (1, 4)],
  'target_beat' : 80,
...
_HARD_RAND_VALUES = {
  ...
  'tb_h_dir_left' : 0.5,
  'wall' : 0.02,
  'wall_length_sec' : [(0.3, 3), (0.6, 4), (1, 5)],
  'target_beat' : 95,
...
_EXPERT_RAND_VALUES = {
  ...
  'tb_h_dir_left' : 0.5,
  'wall' : 0.05,
  'wall_length_sec' : [(0.3, 1), (0.6, 4), (1, 6)],
  'target_beat' : 110,
...
_DIRECTIONS = [
  [[6], [0, 1, 2, 3, 4, 5, 6, 7, 8], [0, 1, 2, 3, 4, 5, 6, 7, 8], [7]], # bottom row
  [[2], [0, 1, 2, 3, 4, 5, 6, 7, 8], [0, 1, 2, 3, 4, 5, 6, 7, 8], [3]], # center row
  [[4], [0], [0], [5]] # top row
]


def required_data():
  return [music_loader.DataType.BEAT]
...
def generate(inp):
  ...
  for difficulty, probabilities in _DIFFICULTIES:
    ...
    wall_length = probabilities['wall_length_sec']
    ...
    while pos < inp.length:
      ...
      if pos > wall_end and rand.random() < prob_wall:
        length = 1
        length_r = rand.random()
        for p, l in wall_length:
          if length_r < p:
            length = l
        # compute the length from seconds to beats 
        length = math.ceil(length * time_to_beats)
        ...
        _compute_wall_probs(wall_rows, wall_cols, wall_prob_row, wall_prob_col, prob_row, prob_col)
